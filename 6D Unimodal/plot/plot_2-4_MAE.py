...
# TNN
path1 = './FPNN_TNN_6D_Unimodal_plot.npy'
...
# MLP
path2 = './FPNN_MLP_6D_Unimodal_plot.npy'
model2 = np.load(path2, allow_pickle=True).item()
p_pred2 = model2['pred'][-1]
mae2 = model2['mae'][-1]

# TFFN
path3 = './TFFN_6D_Unimodal_plot.npy'
...
axes[1, 0].set_title('$(x_1, 0, x_3, 0, 0, 0)$', fontsize=16)
x = X[1][:, 0].reshape(N, N)
y = X[1][:, 2].reshape(N, N)
...
for i in range(2):
    ...
    for j in range(4):
        if i == 0:
            ...
            axes[i, j].set_xlabel('$x_1$', fontsize=12)
            axes[i, j].set_ylabel('$x_3$', fontsize=12)
...
plt.savefig("6D Unimodal MAE.png", dpi=300)
plt.close()