...
# TFFN
path1 = './TFFN_6D_Unimodal_plot.npy'
...
# TNN
path2 = './FPNN_TNN_6D_Unimodal_plot.npy'
model2 = np.load(path2, allow_pickle=True).item()
steps2 = model2['it']
print('FPNN steps: ', steps2)  #[100, 200, 400, 600, 800, 1000, 5000]
p_pred2 = model2['pred']
mae2 = model2['mae']
...
x2 = X[3][:, 1].reshape(N, N)
y2 = X[3][:, 3].reshape(N, N)
...
axes[1, 0].set_title('$(0, x_2, 0, x_4, 0, 0)$', fontsize=16)
axes[1, 0].axis('off')
axes[1, 0] = fig.add_subplot(2, 9, 10, projection='3d')
axes[1, 0].plot_surface(x2, y2, p_true[3].reshape(N, N), cmap='rainbow')
axes[1, 0].set_xlabel('$x_2$', fontsize=10)
axes[1, 0].set_ylabel('$x_4$', fontsize=10)
...
p_pred_FPNN_100 = p_pred2[0]
mae_FPNN_100 = mae2[0]

p_pred_FPNN_400 = p_pred2[2]
mae_FPNN_400 = mae2[2]

vmin = min([d.min() for d in mae_TFFN_1k + mae_TFFN_10k + mae_FPNN_100 + mae_FPNN_400])
vmax = max([d.max() for d in mae_TFFN_1k + mae_TFFN_10k + mae_FPNN_100 + mae_FPNN_400])
...
def plot_2(fig_id, x, y, p, mae):
    ...
    axin = axes[1, fig_id].inset_axes([0.14, 0.07, 0.75, 0.86])
    ...
    if fig_id % 2 == 1:
        ...
        axes[0, fig_id].set_xlabel('$x_2$', fontsize=10)
        axes[0, fig_id].set_ylabel('$x_4$', fontsize=10)
        axin.set_xlabel('$x_2$', fontsize=10, labelpad=-1)
        axin.set_ylabel('$x_4$', fontsize=10, labelpad=-5)
...
plot_2(5, x1, y1, p_pred_FPNN_100[0], mae_FPNN_100[0])
plot_2(6, x2, y2, p_pred_FPNN_100[3], mae_FPNN_100[3])
plot_2(7, x1, y1, p_pred_FPNN_400[0], mae_FPNN_400[0])
plot_2(8, x2, y2, p_pred_FPNN_400[3], mae_FPNN_400[3])
...
fig.text(0.67, 0.90, 'FPNN (TNN), 0.1k steps', ha='center', fontsize=18.5, fontname='Arial', weight='semibold')
fig.text(0.89, 0.90, 'FPNN (TNN), 0.4k steps', ha='center', fontsize=18.5, fontname='Arial', weight='semibold')

plt.savefig("6D Unimodal Steps.png", dpi=300)
plt.close()