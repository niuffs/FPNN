...
def create_Xd(n, xL, xR, device):
    ...
    x3 = torch.cat([zeros, X, Y] + [zeros for _ in range(3)], dim=1)
...
class FPNN(torch.nn.Module):
    ...
    def p_true(self, X):
        H = 3 * ((X[:, 0] ** 4 - X[:, 1]) ** 2 + 2 * X[:, 1] ** 2 + (X[:, 2] ** 4 - X[:, 3]) ** 2 + 2 * X[:, 3] ** 2 + (
                    X[:, 4] ** 4 - X[:, 5]) ** 2 + 2 * X[:, 5] ** 2)
        p = torch.exp(-H) / 1.06257983664097
        return p
    ...
    def train_FPNN(self):
        ...
        for epoch in range(1, self.num_epoch + 1):
            ...
            # Plot
            if epoch % self.plot_freq == 0 or it in [5000, 10000, 15000, 20000]:
                self.plot_fig(it)
    ...
    def plot_fig(self, it):
        ...
        axes[0, 2].set_title('$(0, x_2, x_3, 0, 0, 0)$', fontsize=16)
        ...
        if it in [5000, 10000, 15000, 20000]:
...
if __name__ == "__main__":
    ...
    # Save
    torch.save(model, model_path + '/FPNN_MLP_6D_Unimodal.pth')
    np.save(model_path + '/FPNN_MLP_6D_Unimodal_results.npy', model.results, allow_pickle=True)
    np.save(model_path + '/FPNN_MLP_6D_Unimodal_plot.npy', model.plot, allow_pickle=True)
