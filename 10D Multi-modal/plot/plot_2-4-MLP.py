...
path = './FPNN_MLP_10D_Multi-modal_plot.npy'

model = np.load(path, allow_pickle=True).item()
steps = model['it']
print('FPNN steps: ', steps)  #[500, 1000, 1500, 5000]
p_pred = model['pred']
X = model['x']
p_true = model['true']
...
fig.text(0.44, 0.95, 'FPNN (MLP)', va='center', fontsize=26, fontname='Arial', weight='semibold')
plt.savefig("10D Multi-modal MLP Steps.png", dpi=300)